<?php 
	class ACL {
		...
		function getRole($userID) {
			$strSQL = "SELECT user_roles.userID as userID, user_roles.roleID as roleID, user_roles.addDate as addDate, roles.roleName FROM `user_roles` INNER JOIN roles on user_roles.roleID = roles.ID WHERE `userID` = " . floatval($userID) . " ORDER BY `addDate` DESC";
			$data = mysql_query($strSQL);
			$i=0;
			while ($row = mysql_fetch_array($data)) {
				if (mysql_num_rows($data) > ($i + 1)) {
					echo $row['roleName'].", ";
				} else {
					echo $row['roleName'];
				}
